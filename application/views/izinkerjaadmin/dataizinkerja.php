        ...
        <ul class="list-group list-group-flush">
            ...
            <li class="list-group-item">
                <div class="table-responsive">
                    <table class="table table-striped table-hover" id="tableIzinKerja" width="100%" cellspacing="0">
...
<script type="text/javascript">
    ...
    function getFilter() {
        ...
        $('#tableIzinKerja').DataTable({
            ...
            pageLength: 25,
            ajax : {
                "url": "<?php echo site_url('izinkerjaforadmin/getDataIzinKerja?token=' . $_GET['token']) ?>",
                "type": "POST",
                "data": {
                    "bulan"         : $('#bulan').val(),
                    "skpd_id"       : $('#skpd_id').val(),
    ...
    function hapusizin(meta_id){
        var searchField = $("[type=search][aria-controls=tableIzinKerja]");
        var searchVal   = searchField.val();
        if(!confirm('Apakah anda yakin untuk menghapus?')) return false;
        var url     = "<?=base_url('izinkerjaforadmin/hapusizin');?>";
        var token   = "<?=$_GET['token'];?>";
        $.ajax({
            url     : url+"/"+meta_id+"?token="+token,
            type    : "get",
            success : function(res){
                res = $.parseJSON(res);
                toast(res.pesan, res.warna);
                $('#tableIzinKerja').DataTable().search( searchVal ).draw();
            }
        });
    }
