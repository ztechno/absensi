...
<div class="content-wrapper">
    <div class="card">
        <div class="card-header">
          <h3><?=$title?></h3>
          <div style="position: absolute;right: 8px;top: 6px;"><a href="<?=base_url('pegawai?token='.$_GET['token']);?>" class="btn btn-sm btn-danger"><em class="ti-arrow-left"></em> Kembali</a></div>
        </div>
        <ul class="list-group list-group-flush">
                    ...
                    <?php
                        foreach ($all_files as $key => $file)
                        {
                            ...
                            if (in_array($ext, $supported_format)){
                                ...
                                echo '<button class="btn btn-sm btn-danger btn-gambar-faceabsen" data-index="'.$i.'" style="border-radius:50%; padding: 15px;position:absolute;top:10px;right:20px;"><em class="ti-close"></em></button>';
                                echo "</div>";
                            }else{
                                continue;
                            }
            ...
            <li class="list-group-item">
                <button class="btn btn-primary btn-submit" id="btnSubmit" <?= iterator_count($all_files) ? '' : 'style="display:none"' ?> onclick="train()">Simpan & Perbaharui</button>
            </li>
        </ul>
...
function pushImage(imageSrc)
{
    ...
    imageResponse.innerHTML += "<div class='col-12 col-sm-3' style='margin-bottom:10px'><img id='img"+index+"' class='img-thumbnail' src="+imageSrc+"></div>"
    index++
...
async function train()
{
	...
	var request = await fetch('<?=base_url()?>/pegawai/savefaceabsen/<?=str_replace(" ","",$pegawai['username']);?>?token=<?=$_GET['token']?>',{
		method:'POST',
		body:formData
	})
